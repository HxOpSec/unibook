...
Future<void> seedFirebaseDemoData({required bool enabled}) async {
  if (!enabled) return;
  ...
  try {
    ...
    debugPrint('seedFirebaseDemoData skipped: $e');
  }
}
...
Future<void> seedDepartments() async {
  ...
  for (final dept in _kDepartments) {
    ...
    batch.set(firestore.collection('departments').doc(id), <String, dynamic>{
      'name': dept['name'],
      'code': dept['code'],
...
Future<void> seedDemoBooks() async {
  ...
  for (final book in _kDemoBooks) {
    ...
    batch.set(firestore.collection('books').doc(id), <String, dynamic>{
      'title': book['title'],
      'author': book['author'],
      'year': book['year'],
